...
public class RankCreateCommand extends ISubCommand {

    public RankCreateCommand(String command, ICommand iCommand) {
        super(command, iCommand);
    }

    @Override
    public void onCommand(CommandSender sender, String[] args) throws Exception {
        if(sender.hasPermission("drank.create") || sender.hasPermission("drank.*")){
            // on continue
        }
    }
}
