...
public class RankUserAddCommand extends ISubCommand {

    private DRank dRank;

    public RankUserAddCommand(String command, ICommand iCommand, DRank dRank) {
    ...
    public void onCommand(CommandSender sender, String[] args) throws Exception {
        if(sender.hasPermission("drank.user.add") || sender.hasPermission("drank.*")){
            if(args.length == 2){
                Player target = Bukkit.getPlayer(args[0]);
                if(target != null){
                    new PlayerDataManager(dRank, Bukkit.getPlayer(sender.getName()), target).setRank(args[1]);
                }else{
            ...
            if(args.length == 1){
                sender.sendMessage(RED + "Please specify a rank");
            }
